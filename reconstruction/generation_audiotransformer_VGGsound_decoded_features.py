...
def recon_sound(
  ...
  for sbj, roi, layer in product(subjects, rois, layers):
    ...
    config, sampler, melgan, melception = load_model(model_name, model_dir, lib_path, device) # ここにmodel_nameの代わりにlayerを突っ込む
    
    # For scaling
    y_mean = hdf5storage.loadmat(os.path.join(features_decoders_dir, layer, sbj, roi, 'model', 'y_mean.mat'))["y_mean"]
    y_norm = hdf5storage.loadmat(os.path.join(features_decoders_dir, layer, sbj, roi, 'model', 'y_norm.mat'))["y_norm"]
    ...
    for i, label in enumerate(dec_labels):
      ...
      feat = dec_features.get(layer=layer, subject=sbj, roi=roi, label=label)
      ...
      patch_size_j = 21
      if "fc" in layer:
        visual_features['feature'] = np.array(feat).reshape(1, -1, order='F')
      else:
        #feat = feat[np.newaxis]
        feat = ResampleTemporal(feat)
        visual_features['feature'] = np.array(feat).transpose(3, 0, 1, 2).reshape([patch_size_j, -1])
      ...
      with torch.no_grad():
        ...
        B, D, hr_h, hr_w = sampling_shape = (1, 256, 5, patch_size_j * W_scale)
        ...
        for step in pbar:
          ...
          patch = z_pred_indices.reshape(B, hr_w, hr_h).permute(0, 2, 1)[:, i_start:i_end, j_start:j_end].permute(0, 2, 1).reshape(B, patch_size_i * patch_size_j)
      ...
      soundfile.write(save_wav_path, waves['vocoder'], config.data.params.sample_rate, 'PCM_24')
...
if __name__ == '__main__':
  ...
  feature_decoding_config = os.path.join(conf['feature decoding dir'], conf['feature decoding config'])
