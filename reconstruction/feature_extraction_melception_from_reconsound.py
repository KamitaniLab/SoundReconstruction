...
def extract_feat_from_recon_sound(
    ...
    print('')
    print('Recon data dir:   {}'.format(base_input_dir))
    print('Output directory: {}'.format(base_output_dir))
    print('')

    # Define scaling layer
    def ScalingLayer(inp, lib_path, device):
        # we are gonna use get_ckpt_path to donwload the stats as well
        stat_path = get_ckpt_path('vggishish_mean_std_melspec_10s_22050hz', os.path.join(
            lib_path, 'specvqgan/modules/autoencoder/lpaps'))
    ...
    feature_extractor = FeatureExtractor(
        encoder, eval_layers, eval_layer_mapping, device=device, detach=True)
    ...
    for sbj, roi, layer in product(subjects, rois, layers):
        ...
        recon_labels = [os.path.splitext(os.path.split(f)[1])[
            0] for f in filelist]

        for i, recon_label in enumerate(recon_labels):
            print("Extraction target: {}".format(recon_label))
            ...
            spectrogram = extract_melspectrogram(
                video_path, audio_fps, duration=4)
            spectrogram = {'input': spectrogram}
            random_crop = False
            crop_img_fn = CropImage(
                [config.data.params.mel_num, 336], random_crop)
            ...
            savemat(os.path.join(save_dir, recon_label + '.mat'),
            ...
            for k, v in features.items():
                ...
                savemat(os.path.join(
                    save_dir, recon_label + '.mat'), {"feat": v})
            ...
            for k in featlist:
                ...
                if not os.path.exists(save_dir):
                    os.makedirs(save_dir)
                savemat(os.path.join(
                    save_dir, recon_label + '.mat'), {"feat": v[k]})
